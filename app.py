...
if st.session_state.phase == "INIT":
    ...
    with t2:
        for color, pids in COLOR_GROUPS.items():
            owners = []
            for p in pids:
                owners.append(st.session_state.ownership[p])
            if len(set(owners)) == 1 and owners[0] != "Bank":
                st.markdown(f'<div style="background:{COLOR_MAP[color]}; padding:5px; color:white;">{color} ({owners[0]})</div>', unsafe_allow_html=True)
                for pid in pids:
                    c1, c2, c3 = st.columns([3, 1, 1])
                    c1.write(PROPERTIES[pid]['name'])
                    if c2.button("-", key=f"hm{pid}"):
                        if st.session_state.houses[pid] > 0:
                            st.session_state.houses[pid] -= 1
                            st.rerun()
                    c2.write(f"{st.session_state.houses[pid]}")
                    if c3.button("+", key=f"hp_{pid}"):
                        if st.session_state.houses[pid] < 5:
                            st.session_state.houses[pid] += 1
                            st.rerun()
    ...
    with t3:
        for i, p in enumerate(st.session_state.players):
            ...
            # GOOJF Logic per requirements
            has_ch = False
            has_cc = False
            for card in p['goo_cards']:
                if card['deck'] == "chance": has_ch = True
                if card['deck'] == "chest": has_cc = True
    ...
    for p in st.session_state.players:
        with st.sidebar.expander(f"👤 {p['name']} - ${p['cash']}", expanded=True):
            if p.get('in_jail'):
                st.error("IN JAIL 🚔")
            if p['goo_cards']:
                st.success(f"GOOJF Cards: {len(p['goo_cards'])}")
            for color, pids in COLOR_GROUPS.items():
                owned = []
                for pid in pids:
                ...
                if owned:
                    ...
                    lines = []
                    for pid in owned:
                        lines.append(f"{PROPERTIES[pid]['name']} ({st.session_state.houses[pid]}🏠)")
                    st.write(", ".join(lines))
    ...
    for i, cell in enumerate(top_row):
        cols_t[i+1].write(board_markers[cell])

    for r in range(11):
        cols = st.columns([1] + [1]*11 + [1])
        if 1 <= r <= 9:
            cols[0].write(board_markers[left_col[r-1]])
        ...
        if r == 0:
            ...
            row_data = [left_col[r-1]] + [""]*9 + [right_col[r-1]]
        ...
        if 1 <= r <= 9:
            cols[12].write(board_markers[right_col[r-1]])
    ...
    if curr_p.get('in_jail'):
        ...
        if c1.button("Roll for Doubles"):
            run_turn(jail_action="Try Doubles")
            st.rerun()
        if c2.button("Pay $50"):
            run_turn(jail_action="Pay $50")
            st.rerun()
        if c3.button("Use GOOJF Card", disabled=not curr_p['goo_cards']):
            run_turn(jail_action="Use Card")
            st.rerun()
    else:
